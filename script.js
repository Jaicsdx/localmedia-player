(() => {
  ...
  function renderMovies(){
    ...
    els.grid.innerHTML = list.map(m => {
      ...
      const dateTag = new Date(m.addedAt||Date.now()).toLocaleDateString();
      return `
  ...
  function startEdit(m){
    ...
    els.poster.value = m.poster || '';
    els.addBtn.disabled = true;
    els.updateBtn.disabled = false;
    els.cancelEditBtn.disabled = false;

    // On mobile, open sidebar so user sees fields
  ...
  els.addBtn?.addEventListener('click', () => {
    ...
    const movie = { id: crypto.randomUUID(), name, url: link, poster: poster || undefined, addedAt: Date.now() };
  ...
  els.updateBtn?.addEventListener('click', () => {
    ...
    if(i>=0){ state.movies[i] = { ...state.movies[i], name, url: link, poster: poster || undefined }; }
    saveLocal();
    renderMovies();
    clearEditor();
  });
  ...
  (async function init(){
    ...
    // Try remote JSON first; if missing, fallback to local store.
    const remote = await loadRemoteJSON();
    if(remote){
      // Upgrade any legacy entries that have no poster
